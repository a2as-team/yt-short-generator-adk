import os
import logging
import requests
import tempfile
from typing import List, Optional
from urllib.parse import quote

# Configure logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

async def search_images_online(query: str, num_images: int = 5, api_key: Optional[str] = None) -> List[str]:
    """
    Search for images online related to the query.
    In a production environment, you would use a proper API like Google Custom Search.
    
    Args:
        query: The search query
        num_images: Number of images to fetch
        api_key: Google API key (optional)
        
    Returns:
        List of local file paths to downloaded images
    """
    logger.info(f"Searching for images with query: {query}")
    
    # Use the provided API key or get from environment
    api_key = api_key or os.getenv("GOOGLE_API_KEY")
    
    # In a real implementation, you would use Google Custom Search API or similar
    # For demo purposes, we'll create placeholder images with the query text
    
    # Create a temp directory to store images
    temp_dir = tempfile.mkdtemp()
    image_paths = []
    
    try:
        # For demo purposes, we'll use Unsplash API
        # If we had a Google API key, we would use Custom Search API instead
        encoded_query = quote(query)
        url = f"https://api.unsplash.com/photos/random?query={encoded_query}&count={num_images}"
        
        # Check if we have an Unsplash access key in environment
        unsplash_key = os.getenv("UNSPLASH_ACCESS_KEY")
        
        if unsplash_key:
            # Make API request
            headers = {"Authorization": f"Client-ID {unsplash_key}"}
            response = requests.get(url, headers=headers)
            
            if response.status_code == 200:
                images = response.json()
                
                for i, img in enumerate(images):
                    # Download the image
                    img_url = img["urls"]["regular"]
                    img_path = os.path.join(temp_dir, f"image_{i}.jpg")
                    
                    img_response = requests.get(img_url)
                    if img_response.status_code == 200:
                        with open(img_path, "wb") as f:
                            f.write(img_response.content)
                        image_paths.append(img_path)
            else:
                logger.warning(f"Failed to fetch images from Unsplash: {response.status_code}")
        else:
            logger.warning("No Unsplash API key found, using placeholder images")
    
    except Exception as e:
        logger.error(f"Error searching for images: {e}")
    
    # If we couldn't get real images, create placeholders
    if not image_paths:
        from PIL import Image, ImageDraw, ImageFont
        
        # Create placeholder images with the query text
        for i in range(num_images):
            img_path = os.path.join(temp_dir, f"placeholder_{i}.jpg")
            
            # Create a new image
            img = Image.new('RGB', (1080, 1080), color=(0, 0, 0))
            draw = ImageDraw.Draw(img)
            
            # Use a default font
            try:
                font = ImageFont.truetype("arial.ttf", 40)
            except IOError:
                font = ImageFont.load_default()
            
            # Add text
            text = f"{query} - Image {i+1}"
            text_width = draw.textlength(text, font=font)
            text_x = (1080 - text_width) / 2
            text_y = 1080 / 2
            
            draw.text((text_x, text_y), text, fill="white", font=font)
            
            # Save the image
            img.save(img_path)
            image_paths.append(img_path)
    
    return image_paths 